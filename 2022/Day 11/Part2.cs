...
class Monkey
{
    ...
    public void TestAll(long commonDiv)
    {
        while (Items.Any())
        {
            var item = Items[0];
...
foreach (var ln in lines)
{
    ...
    if (m.Success)
    {
        cur.Items.AddRange(m.Groups[1].Value.Split(',').Select(i => long.Parse(i.Trim())));
        continue;
    }
...
var commonDiv = monkeys.Values.Aggregate(1, (a, b) => a * b.TestDivisor);

for (var round = 1; round <= 10_000; ++round)
{
    foreach (var monkey in monkeys.Values)
    {
        monkey.TestAll(commonDiv);
    }
}

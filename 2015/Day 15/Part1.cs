var props = new [] { "capacity", "durability", "flavor", "texture" /*, "calories"*/ };
...
while ((ln = Console.In.ReadLine()) != null)
{
    ...
    ingredients.Add(props.ToDictionary(p => p, p => int.Parse(m.Groups[p].Value)));
}
...
foreach (var perm in perms)
{
    ...
    var values = props.Select(p => Enumerable.Range(0, ingredients.Count).Select(i => spoons[i] * ingredients[i][p]).Sum());
